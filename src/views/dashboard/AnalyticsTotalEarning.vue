...
// Statistiques frais d’inscription (payés, en attente, refusés)
const earnings = computed(() => {
  if (!paymentStats.value) return []

  const trainingPayments = paymentStats.value.training_payments || {}

  const approved = trainingPayments.APPROVED?.registration_fees || { total_amount: 0, paid_count: 0 }
  const received = trainingPayments.RECEIVED?.registration_fees || { total_amount: 0, total_count: 0 }
  const refused = trainingPayments.REFUSED?.registration_fees || { total_amount: 0, total_count: 0 }

  return [
    {
      avatar: 'ri-checkbox-circle-line',
      title: 'Frais d\'inscription payés',
    ...
    {
      avatar: 'ri-time-line',
      title: 'Frais d\'inscription en attente',
    ...
    },
    {
      ...
      title: 'Frais d\'inscription refusés',
...
// Total payé uniquement
const totalEarnings = computed(() => {
  if (!paymentStats.value) return 0
  const approved = paymentStats.value.training_payments?.APPROVED?.registration_fees || {}
...
<template>
  <VCard>
    <VCardItem>
      <VCardTitle>Inscriptions aux Formations</VCardTitle>
    </VCardItem>
    ...
    <VCardText>
      ...
      <div class="text-body-1 mb-12">
        Revenus des frais d'inscription aux formations.
      </div>
